...
void results::dump_shard(uint64_t t, const double2* shard) const {
	...
	MPI_File dump_mpif;
	MPI_File_open(MPI_COMM_WORLD, dumpname.c_str(), MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &dump_mpif) && assertmpi;
...
void results::check_entropy(const array<double, 2> &entropies) const {
	...
	switch(gconf.entropy_limit_type){
		case configuration::NONE: return;
		case configuration::WT: if(entropies[0] >= gconf.entropy_limit) return;
		case configuration::INFORMATION: if(entropies[1] >= gconf.entropy_limit) return;
	}
	quit_requested = true;
}
