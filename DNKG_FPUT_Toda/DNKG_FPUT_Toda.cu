...
namespace DNKG_FPUT_Toda {
	...
	__global__ void move_chain_in_warp(double2 *planar, uint32_t steps_grouping, uint16_t copies) {
		...
		planar += full_lane ? lane * elements_in_thread : full_lanes * elements_in_thread +
	...
	__global__ void move_chain_in_thread(double2 *planar, uint32_t steps_grouping, uint16_t copies) {
		...
		for (uint32_t i = 0; i < steps_grouping; i++) {
			...
			for (int k = 0; k < 7; k++) {
				...
				pairs[chain_length - 1].y +=
						dt_d[k] * rhs(pairs[chain_length - 2].x, pairs[chain_length - 1].x, pairs[0].x);
			}
		}
		#pragma unroll
		for (uint16_t i = 0; i < chain_length; i++) {
	...
	template<Model model>
	__global__ void
	move_split(double *phi, double *pi, uint32_t chainlen, uint32_t shard_copies, uint32_t steps_grouping) {
